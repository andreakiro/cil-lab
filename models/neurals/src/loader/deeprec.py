    ...
    def iterate_one_epoch(self):
        ...
        while e_ind < len(keys):
            ...
            mini_batch = torch.sparse.FloatTensor(
                i_torch, v_torch, torch.Size([self._batch_size, self._vector_dim])
            )
        ...
        while s_ind < len(keys):
            ...
            mini_batch = (
                torch.sparse.FloatTensor(
                    i_torch, v_torch, torch.Size([1, self._vector_dim])
                ),
                torch.sparse.FloatTensor(
                    src_i_torch, src_v_torch, torch.Size([1, self._vector_dim])
                ),
            )
            s_ind += 1
            if not for_inf:
