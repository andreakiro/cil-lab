...
def preprocess_data():
    ...
    raw_data = pd.read_csv(register.datapaths.cil)
    number_of_users, number_of_movies = config.NUM_USERS, config.NUM_MOVIES
    ...
    for user, movie, rating in zip(train_users, train_movies, train_ratings):
        filled_training_matrix[user - 1][movie - 1] = rating
        training_mask[user - 1][movie - 1] = 1
    ...
    for user, movie, rating in zip(val_users, val_movies, val_ratings):
        filled_validation_matrix[user - 1][movie - 1] = rating
        val_mask[user - 1][movie - 1] = 1
    ...
    sub_pd = pd.read_csv(register.datapaths.cil_sample)
